...
namespace Spark
{
    void Window::Init(const char *name, unsigned int width, unsigned int height)
    {
        ...
        SPARK_CHECK(glfwInit() > 0, "Failed to initialize GLFW");
        m_window = glfwCreateWindow(width, height, name, nullptr, nullptr);
        SPARK_CHECK(m_window != nullptr, "Failed to create GLFW window");

        glfwMakeContextCurrent(m_window);
        SPARK_CHECK(gladLoadGLLoader((GLADloadproc)glfwGetProcAddress) > 0, "Failed to initialize GLAD");
    }

    void Window::Shutdown()
    {
        SPARK_ASSERT(IsValid(), "Window::Shutdown: Invalid window");
    ...
    {
        SPARK_ASSERT(IsValid(), "Window::Shutdown: Invalid window");
        SPARK_ASSERT(listener != nullptr, "Window::SetupEventsCallback: Invalid parameter");
